...
def index(request):
    ...
    if request.POST:
        try:
            lsSerialId = request.POST["txtListSerial"];
            arrSerialId = lsSerialId.split(',')
            p_type= '2'
            user_name = request.user.username
            for serialId in arrSerialId :
                stockAssetSerial =StockAssetSerial.objects.get(id = serialId)
                p_parent_serial = stockAssetSerial.parent_serial
                p_serial = stockAssetSerial.parent_serial
#                 cursor = connection.cursor()
#                 cursor.execute("ALTER SESSION SET NLS_DATE_FORMAT = 'DD/MM/YYYY HH24:MI:SS' "  
#                                            "NLS_TIMESTAMP_FORMAT = 'DD/MM/YYYY HH24:MI:SS.FF'")
#                 p_error = cursor.var(cx_Oracle.STRING).var
#                 cursor.callproc("pck_stock_trans.join_release_asset",
#                             (
#                                 # p_error
#                                 p_error,
#                                 # p_type
